...
const schedule = async (req: Request, res: Response) => {
  const { hours, minutes, count, resume_id, message, userId, email } = req.body;

  // 1. Смотрю есть ли уже запущенная таска
  const job = await agenda.jobs({ name: userId });

  if (!job.length) {
    // 2. Если нет, то создаю таску с названием айди пользователя

    agenda.define(userId, async (job, done) => {
      try {
    ...
    await agenda.every(`${minutes} ${hours} * * *`, userId, req.body, {
      timezone: "Europe/Moscow",
    });

    agenda.on("complete", (job) => {
      // 7. По завршению таски и если есть отклики то отправляю сообщение на почту
      if (job.attrs.name === userId && stack.length) {
    ...
    async function graceful() {
      await agenda.stop();
      await agenda.cancel({ name: userId });
      process.exit(0);
    }
